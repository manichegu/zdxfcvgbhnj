...
function EnterPodcasts() {
  ...
  async function submit(e){
    ...
    try{
      await axios.post("http://localhost:8000/enterPodcast",{
        podcastname,category,speaker,description
      })
      .then(res=>{
        if(res.data==="done"){
            alert("done!!");
            // history("/enterPodcast");
        }
        else if(res.data==="notdone"){
            alert("notdone!!");
        }
        else{
            alert("notdone!!");
...
<div className="session">
        ...
        <form action="post" className="log-in" autoComplete="off"> 
          <h4>We are <span>NUVA</span></h4>
          <p>Welcome back! Log in to your account to view today's clients:</p>
          <div className="floating-label">
            <input required placeholder="Podcastname" type="text"  id="email" autoComplete="off"   onChange={(e)=>{setPodcastname(e.target.value)}}/>
            <label htmlFor="email">Podcastname</label>
            <div className="icon">
              {/*?xml version="1.0" encoding="UTF-8"?*/}
          ...
          <div className="floating-label">
            <input required placeholder="Category" type="text" name="password" id="password" autoComplete="off" onChange={(e)=>{setCategory(e.target.value)}} />
          ...
          <div className="floating-label">
            <input required placeholder="Speaker" type="text" name="password" id="password" autoComplete="off" onChange={(e)=>{setSpeaker(e.target.value)}} />
            <label htmlFor="password">Speaker</label>
            <div className="icon">
              {/*?xml version="1.0" encoding="UTF-8"?*/}
              <svg enableBackground="new 0 0 24 24" version="1.1" viewBox="0 0 24 24" xmlSpace="preserve" xmlns="http://www.w3.org/2000/svg">
                <style type="text/css" dangerouslySetInnerHTML={{__html: "\n\t.st0{fill:none;}\n\t.st1{fill:#010101;}\n" }} />
          ...
          <div className="floating-label">
            <textarea required placeholder="Description" type="text" name="password" id="password" autoComplete="off"  onChange={(e)=>{setDescription(e.target.value)}} />
          ...
          {/* <div className="floating-label">
            <input required placeholder="upload" type="file" name="password" id="password" autoComplete="off"  />
